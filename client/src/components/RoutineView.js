...
import { Table, Container, Alert, Spinner, Form, Row, Col, Button, Badge } from 'react-bootstrap';
...
const RoutineView = () => {
    ...
    const fetchRoutine = useCallback(async () => {
        if (selectedProgram && selectedSection) {
            ...
            try {
                ...
                console.error("Fetch Routine Error:", err.response?.data || err.message);
                setRoutine({});
    ...
    const routinePDF = () => { // Simplified PDF generation
        const element = document.getElementById("routine-table-container"); // Target the container div
        if (!element) return;
                            ...
                            {days.map(day => {
                                ...
                                if (dayRoutine) {
                                    ...
                                    for (const actualSlotKey in dayRoutine) {
                                        ...
                                        const [actualStartTimeStr, actualEndTimeStr] = actualSlotKey.split(' - ');
                                        ...
                                        if (actualStartTimeStr === displaySlot.start) {
                                            ...
                                            cellRowSpan = calculateRowSpan(actualStartTimeStr, actualEndTimeStr);
                                            ...
                                            const isLab = cellRowSpan > 1 || slotData.course_type === 'Lab'; // Also check actual type if available
                                ...
                                return (
                                    <td key={`${day}-${displaySlot.start}`} className="routine-cell" rowSpan={cellRowSpan}>
                                        {cellContent}
                                    </td>
                                );
                            })}
    ...
    return (
        <Container fluid className="mt-4">
            ...
            <Row className="mb-4 justify-content-center">
                ...
                <Form.Group controlId="programSelect" className="mb-3">
                            ...
                            <Form.Select value={selectedProgram} onChange={(e) => {
                                setSelectedProgram(e.target.value); setSelectedSection(''); setRoutine(null);
                            }}>
                            <option value="">-- Select Program --</option>
                            {Object.keys(programs).map(program => (<option key={program} value={program}>{program}</option>))}
                        </Form.Select>
                    </Form.Group>
                </Col>
                <Col md={4}>
            ...
            {loading && <Container className="d-flex justify-content-center align-items-center" style={{ minHeight: '200px' }}><Spinner animation="border" /></Container>}
            {!loading && error && <Container className="mt-4"><Alert variant="danger">{error}</Alert></Container>}
            ...
            {!loading && !error && routine && selectedProgram && selectedSection && (
                        ...
                        <Table bordered hover className="routine-table text-center">
                            <thead /* className="table-dark" - Removed dark header for better PDF/Image capture */>
                                <tr>
                                    ...
                                    {days.map(day => (
                                        <th key={day} style={{ width: '17%', backgroundColor: '#f8f9fa' }}>{day}</th>
                                    ))}
                                </tr>
                            </thead>
                            {renderTableBody()}
                ...
                <div className="text-center mb-4">
                    <Button variant="primary" className="me-3" onClick={routineImage}> Download Image </Button>
                    <Button variant="success" onClick={routinePDF}> Download PDF </Button>
                </div>
                </>
            )}
