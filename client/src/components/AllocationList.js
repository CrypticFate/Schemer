...
const AllocationList = ({ refresh }) => {
  ...
  const getAllocations = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || "Error fetching allocations.");
      setAllocations({});
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      log("INFO", "Successfully deleted allocation:", id);
      getAllocations(); // Refresh the list
    } catch (err) {
      log("ERROR", "Error deleting allocation:", err);
  ...
  const availableDays = Object.keys(allocations).filter(
    (day) => allocations[day]?.length > 0
  );
    ...
    <div>
      <Card className="shadow-sm">
        ...
        <Card.Body className="p-4">
          ...
          {!loading &&
          selectedDay &&
          Array.isArray(allocations[selectedDay]) &&
          allocations[selectedDay].length > 0 ? (
            <div className="table-responsive mt-3">
              <Table
                ...
                hover
                size="sm"
                className="align-middle text-center allocations-table"
              >
                {" "}
                {/* Added class */}
                <thead className="table-secondary">
                  {" "}
                  {/* Use secondary for header */}
                  <tr>
                ...
                <tbody>
                  {allocations[selectedDay].map((alloc) => (
                    <tr key={alloc.allocation_id}>
                      <td>
                        <span className="d-block time-display">
                          {formatTime(alloc.start_time)} -
                        </span>
                        <span className="d-block time-display">
                          {formatTime(alloc.end_time)}
                        </span>
                      </td>
                      <td>
                        ...
                        <Badge
                          pill
                          bg={
                            alloc.course_type === "Lab" ? "primary" : "success"
                          }
            ...
            <Alert variant="secondary" className="text-center mt-3">
              No allocations found for {selectedDay}
            </Alert>
          ) : !loading ? (
            <Alert variant="secondary" className="text-center mt-3">
              Please select a day to view allocations
            </Alert>
          ) : null}
        </Card.Body>
      </Card>
