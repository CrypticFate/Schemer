...
const AllocationForm = ({ onAllocationCreated }) => {
    ...
    try {
      ...
      setCourses(
        Array.isArray(coursesRes.data)
          ? coursesRes.data.filter((c) => c.allocation_availability > 0)
          : []
      );
      ...
      if (err.response) {
        errorDetails = `Status: ${err.response.status}, Data: ${JSON.stringify(
          err.response.data
        )}`;
      } else if (err.request) {
      ...
      setError("Error loading initial form data.");
      setTeachers([]);
      setCourses([]);
      setDays([]);
    } finally {
      log("INFO", "loadInitialData: Finally block reached.");
      ...
      try {
        ...
        if (courseData) {
          ...
          const teacherRes = await axios.get(
            `http://localhost:5000/api/get-teacher-by-course/${courseId}`
          );
          ...
          const sectionsResponse = await axios.get(
            `http://localhost:5000/api/courses/${courseId}/available-sections`
          );
          if (Array.isArray(sectionsResponse.data)) {
            setAvailableSections(sectionsResponse.data);
          } else {
            log("ERROR", "Invalid section data", sectionsResponse.data);
            setAvailableSections([]);
  ...
  const fetchAvailableDays = useCallback(async (courseId, selectedSection) => {
    ...
    log(
      "DEBUG",
      `FETCH_DAYS_START: course ${courseId}, section ${selectedSection}`
    );
    setLoadingDays(true);
    setAvailableDays([]);
    setError("");
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        log("ERROR", "FETCH_DAYS_INVALID_DATA:", response.data);
        setAvailableDays([]);
        setError("Failed to load valid day data.");
      }
    } catch (err) {
      log("ERROR", "FETCH_DAYS_ERROR:", err.response?.data || err.message);
      setError("Error loading available days.");
      setAvailableDays([]);
    } finally {
      log("DEBUG", "FETCH_DAYS_FINALLY: Setting loadingDays to false");
  ...
  const fetchAvailableTimeSlots = useCallback(
    async (dayId, selectedSection, selectedProgram, courseId) => {
      if (!dayId || !selectedSection || !selectedProgram || !courseId) return;
      setLoadingTimeSlots(true);
      setAvailableTimeSlots([]);
      setFormData((prev) => ({ ...prev, slot_id: "", room_id: "" }));
      setAvailableRooms([]);
      try {
        const response = await axios.get(
        ...
        if (Array.isArray(response.data)) {
          ...
          log("ERROR", "API did not return array for slots", response.data);
          setAvailableTimeSlots([]);
          setError("Failed to load valid time slot data.");
        }
      } catch (err) {
        setError("Error loading available time slots.");
        log("ERROR", "Error fetching time slots:", err);
        setAvailableTimeSlots([]);
      } finally {
  ...
  const loadAvailableRooms = useCallback(async (dayId, slotId, type) => {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        log("ERROR", "API did not return array for rooms", response.data);
        setAvailableRooms([]);
        setError("Failed to load valid room data.");
      }
    } catch (err) {
  ...
  useEffect(() => {
    fetchCourseData(formData.course_id);
  }, [formData.course_id, fetchCourseData]);
  useEffect(() => {
    if (formData.course_id && section) {
      ...
      setAvailableDays([]);
      if (loadingDays) {
        log("DEBUG", "Clearing stuck loadingDays.");
        setLoadingDays(false);
      }
    }
  ...
  const courseOptions = useMemo(() => {
    ...
    return courses.map((course) => ({
      ...
      label: `${course.course_code} - ${course.course_name} (${course.course_type})`,
    }));
  }, [courses]);
  const selectedCourseOption = useMemo(
    () =>
      ...
      null,
    [formData.course_id, courseOptions]
  );

  // --- Handlers ---
  const handleInputChange = (e) => {
    // Handles non-Select inputs
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !submissionData.teacher_id ||
      !submissionData.course_id ||
      !submissionData.room_id ||
      !submissionData.day_id ||
      !submissionData.slot_id ||
      !submissionData.program ||
      isNaN(submissionData.section)
    ) {
    ...
    try {
      ...
      log("ERROR", "Form Submit: Error received", {
      ...
      if (
        serverErrorMessage.includes("Section") &&
        serverErrorMessage.includes("classes on this day")
      ) {
        // Section allocation limit error
        ...
        setAvailableTimeSlots([]);
        setAvailableRooms([]);
      } else if (serverErrorMessage.includes("workload")) {
        // Teacher workload error
        setWorkloadWarning(serverErrorMessage);
      } else if (err.response?.status === 409) {
        // Conflict error (teacher/room already booked)
        setWorkloadWarning(serverErrorMessage);
        setFormData((prev) => ({
          ...prev,
          day_id: "",
        ...
        setError(
          errorDetails
            ? `${serverErrorMessage} (${errorDetails})`
            : serverErrorMessage
        );
      }
    } finally {
  ...
  const getSectionOptions = () => {
    // Generates section dropdown options
    if (
      !program ||
        ...
        <Card.Body>
          {loadingData ? (
            ...
            <>
              ...
              {workloadWarning && (
                <Alert variant="warning">{workloadWarning}</Alert>
              )}
              ...
              <Form onSubmit={handleSubmit}>
                <div className="allocation-progress mb-4">
                  ...
                  />
                  <div
                    className={`progress-step ${section ? "completed" : ""}`}
                  />
                  <div
                    className={`progress-step ${
                      formData.day_id ? "completed" : ""
                    }`}
                  />
                  <div
                    className={`progress-step ${
                      formData.slot_id ? "completed" : ""
                    }`}
                  />
                  <div
                    className={`progress-step ${
                      formData.room_id ? "completed" : ""
                    }`}
                  />
                </div>
                <Row className="mb-3">
                  ...
                  <Col md={6}>
                    <Form.Group>
                      <Form.Label className="fw-bold">
                        Section{" "}
                        {loadingSections && formData.course_id && (
                      ...
                      >
                        ...
                        {getSectionOptions().map((sec) => (
                          <option key={sec.number} value={sec.number}>
                            {" "}
                            Section {sec.number} ({sec.remaining} slot
                            {sec.remaining !== 1 ? "s" : ""} needed){" "}
                          </option>
                        ))}
                      </Form.Select>
                    </Form.Group>
                        ...
                        {!loadingTimeSlots &&
                          ...
                          availableTimeSlots.map((slot) => (
                            <option key={slot.slot_id} value={slot.slot_id}>
                              ...
                              {slot.end_time.slice(0, 5)} ({slot.slot_type}){" "}
                            </option>
                          ))}
                      </Form.Select>
                    </Form.Group>
