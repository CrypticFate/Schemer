...
app.post("/api/allocations", async (req, res) => {
  ...
  try {
    ...
    if (err.code === "23505") {
      ...
      err.message?.includes("classes on this day")
    ) {
      // Section allocation limit error
      log("ERROR", `Section Allocation Limit Error: ${err.message}`);
      res.status(400).json({ error: err.message });
    } else if (err.message?.includes("workload")) {
      // Teacher workload error
      log("ERROR", `Teacher Workload Error: ${err.message}`);
      res.status(400).json({ error: err.message });
    } else if (
      ...
      log("ERROR", `Database Constraint Error: ${err.message}`);
      res.status(400).json({ error: err.message });
    } else {
      // Unknown error
      log("ERROR", "Unknown Server Error:", err);
      res.status(500).json({
        error: "Internal server error creating allocation.",
        details: err.message,
      });
    }
  } finally {
